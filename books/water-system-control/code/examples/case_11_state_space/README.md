...
<table>
<tr>
<td width="50%"><img src="state_space_diagram.png" alt="状态空间方法示意图" width="100%"/></td>
<td width="50%">

**系统架构说明：**

这张示意图展示了现代控制理论中的状态空间方法及其控制系统设计思路：

**顶部左：状态空间模型（State-Space Model）**：
- **状态方程**：dx/dt = Ax + Bu
- **输出方程**：y = Cx + Du
- **x**：状态向量（系统内部状态）
- **u**：输入（控制信号）
...
**顶部右：状态反馈控制（State Feedback Control）**：
- **控制律**：u = -Kx + r
- **闭环系统**：dx/dt = (A-BK)x + Br
- **K**：状态反馈增益矩阵
- **r**：参考输入
- 通过反馈所有状态实现控制

**中部：系统框图**：
- **被控对象（Plant）**：dx/dt=Ax+Bu, y=Cx
- **状态反馈（State Feedback）**：u=-Kx+r
- **反馈回路**：所有状态x都反馈到控制器
- **参考输入r**：设定值
- **输出y**：被控量
- 完整的状态反馈闭环系统
...
<tr>
<td width="50%">
![分析图](case11_state_space_summary.png)
</td>
<td width="50%">
**分析图说明**
**系统分析**
