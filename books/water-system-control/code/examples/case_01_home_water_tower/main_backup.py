...
def main():
    ...
    # ========================================================================
    # Part 2: On-Off Controller Design
    # ========================================================================
    print(f"\n{'-'*70}")
    print("Part 2: On-Off Controller Design")
    print("-"*70)

    controller = OnOffController(
        low_threshold=2.5,   # Lower threshold
        high_threshold=3.5,  # Upper threshold
        output_on=1.0,       # Pump ON output
        output_off=0.0       # Pump OFF output
    )

    print(f"\nController Parameters:")
    print(f"  Lower threshold (pump ON) = {controller.low_threshold:.1f} m")
    print(f"  Upper threshold (pump OFF) = {controller.high_threshold:.1f} m")
    print(f"  Hysteresis width (dead band) = {controller.high_threshold - controller.low_threshold:.1f} m")
    print(f"\nControl Logic:")
    print(f"  - When water level < 2.5m, pump ON")
    print(f"  - When water level > 3.5m, pump OFF")
    print(f"  - Between 2.5-3.5m, maintain previous state (hysteresis)")
